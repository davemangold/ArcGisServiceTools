...
import json
import logging
from utility import geom_esri_to_geojson

logger = logging.getLogger('FeatureRetriever')
...
class FeatureRetriever(object):
    ...
    def __feature_json_to_geojson(self, feature, feature_type):
        """Return input ArcGIS JSON feature reconfigured as GeoJSON."""

        feature['type'] = 'Feature'
        feature['properties'] = feature.pop('attributes')
        feature['geometry']['type'] = feature_type  # Point, LineString, Polygon
    ...
    def retrieve(self):
        ...
        src_config = self.src_feat_layer.svc_config

        request_args = {'where': src_config['where'],
                        'outFields': src_config['out_fields']}
        if src_config['geometry'] is not None:
            request_args['geometry'] = str(src_config['geometry'])
            request_args['geometryType'] = str(src_config['geometry_type'])

        json_features = self.src_feat_layer.query_features_batch(**request_args)

        # json_features = self.src_feat_layer.query_features_batch(
        #     where=src_config['where'],
        #     geometry=str(src_config['geometry']),
        #     geometryType=src_config['geometry_type'],
        #     outFields=src_config['out_fields'])

        if self.tgt_format == 'esrijson':
            container = self.__get_esri_json_container(src_config['out_fields'])
